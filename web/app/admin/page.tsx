...
type Person = { id: number; name: string; activeWeekly: boolean; activeBiweekly: boolean; shameCount?: number; exceptions?: string[]; };
...
export default function AdminPage() {
  ...
  // State for the new popups
  const [countsOpen, setCountsOpen] = useState(false);
  const [shameListOpen, setShameListOpen] = useState(false);
  const [exceptionsOpen, setExceptionsOpen] = useState(false);

  // State for the new inline vacation picker
  ...
  /* ---------- tiny helper to toggle a single task exception for a person ---------- */
  async function toggleException(person: Person, taskSlug: string) {
    const current = new Set(person.exceptions ?? []);
  ...
  // Vacation Picker Logic
  async function handleSaveVacation() {
    if (vacationPersonId === null || vacationStartWeek === null || vacationEndWeek === null) {
      alert("Bitte Person und Datumsbereich auswählen.");
      return;
    }
    if (!plan?.startsOn) return;

    const start = new Date(plan.startsOn);
    const end = new Date(plan.startsOn);
    start.setDate(start.getDate() + (vacationStartWeek * 7));
    ...
    <main className="p-6">
      ...
      <div className="mb-4 flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            className="border rounded px-3 py-1"
            onClick={() => setShameListOpen(true)}
            title="Schämtliliste anzeigen"
          >
            Schämtliliste
          </button>
      ...
      {/* Inline Urlaub Picker */}
      {vacationPickerOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-lg p-6 w-[400px]">
            <h3 className="text-base font-semibold mb-4">Urlaub für eine Person festlegen</h3>
            <div className="flex flex-col gap-3">
              <label className="block">
                ...
                <select
                  ...
                  onChange={(e) => setVacationPersonId(Number(e.target.value))}
                >
              ...
              <label className="block">
                <span className="text-sm">Start-Woche:</span>
                <select
                  className="w-full border rounded px-2 py-1 mt-1"
                  value={vacationStartWeek ?? ''}
                  onChange={(e) => setVacationStartWeek(Number(e.target.value))}
                >
                  <option value="" disabled>Woche auswählen</option>
                  {Array.from({ length: 16 }, (_, i) => (
                    <option key={i} value={i}>
                      Woche {i + 1} ({mondayLabel(i)})
                    </option>
                  ))}
                </select>
              </label>
              ...
              <label className="block">
                <span className="text-sm">End-Woche:</span>
                <select
                  className="w-full border rounded px-2 py-1 mt-1"
                  value={vacationEndWeek ?? ''}
                  onChange={(e) => setVacationEndWeek(Number(e.target.value))}
                >
                  <option value="" disabled>Woche auswählen</option>
                  {Array.from({ length: 16 }, (_, i) => (
                    <option key={i} value={i} disabled={i < (vacationStartWeek ?? 0)}>
                      Woche {i + 1} ({mondayLabel(i)})
                    </option>
                  ))}
                </select>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                ...
                disabled={vacationPersonId === null || vacationStartWeek === null || vacationEndWeek === null}
              >
                Speichern
              </button>
            </div>
      ...
      {shameListOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          onClick={() => setShameListOpen(false)}
        >
          {/* backdrop */}
          <div className="absolute inset-0 bg-black/40" />
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold">Schämtliliste</h3>
              <button
                className="border rounded px-2 py-1 text-sm"
                onClick={() => setShameListOpen(false)}
                aria-label="Schliessen"
      ...
      {/* Ausnahmen Popup (renamed and simplified) */}
      {exceptionsOpen && (
  <div
    className="fixed inset-0 z-50 flex items-center justify-center"
    role="dialog"
  ...
  >
    ...
    {/* modal */}
    <div
      className="relative bg-white rounded-lg shadow-lg w-[min(95vw,900px)] max-h-[85vh] overflow-auto p-4"
      onClick={(e) => e.stopPropagation()}
    >
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-base font-semibold">Ausnahmen (Task-Opt-outs)</h3>
        <button
          className="border rounded px-2 py-1 text-sm"
          onClick={() => setExceptionsOpen(false)}
          aria-label="Schliessen"
      ...
      <p className="text-sm text-gray-600 mb-3">
        Hake Aufgaben an, die die Person <b>nicht</b> machen soll. (Keine Daten — nur Task-Ausnahmen.)
      </p>
      ...
      <div className="space-y-4">
        {people.map((p) => (
          <div key={p.id} className="border rounded p-3">
            ...
            {/* split tasks by cadence just for visual grouping */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <div className="text-xs font-semibold mb-1 text-gray-700">Wöchentliche Aufgaben</div>
                <div className="flex flex-col gap-1">
                  {tasks.filter(t => t.cadence === 'weekly').map(t => {
                    const checked = (p.exceptions ?? []).includes(t.slug);
                    return (
                      <label key={t.slug} className="flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => toggleException(p, t.slug)}
                        />
              ...
              <div>
                <div className="text-xs font-semibold mb-1 text-gray-700">Zweiwöchentliche Aufgaben</div>
                <div className="flex flex-col gap-1">
